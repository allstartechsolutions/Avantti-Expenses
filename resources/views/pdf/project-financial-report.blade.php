...
<head>
    ...
    <title>{{ __('Project Financial Report') }} — {{ $project->project_name }}</title>
</head>
<body style="font-family: DejaVu Sans, sans-serif; font-size: 9pt; line-height: 1.4; color: #333; margin: 0; padding: 15px;">
    ...
    <table style="width: 100%; border: none; margin-bottom: 12px; border-bottom: 2px solid #3F5189; padding-bottom: 8px;">
        <tr>
            ...
            <td style="width: 50%; vertical-align: top; text-align: right; border: none; padding: 0;">
                <div style="font-size: 14pt; font-weight: bold; color: #3F5189;">{{ __('PROJECT FINANCIAL REPORT') }}</div>
                <div style="font-size: 9pt; color: #555;">{{ $project->project_name }}</div>
    ...
    <table style="width: 100%; border: none; margin-bottom: 12px; font-size: 8pt;">
        <tr>
            <td style="width: 33%; border: none; padding: 2px 0;">
                <strong style="color: #555;">{{ __('Client') }}:</strong> {{ $project->client?->company_name ?? '—' }}
            </td>
            <td style="width: 33%; border: none; padding: 2px 0;">
                <strong style="color: #555;">{{ __('Status') }}:</strong> {{ $project->status->label() }}
            </td>
            <td style="width: 34%; border: none; padding: 2px 0;">
                <strong style="color: #555;">{{ __('Project Manager') }}:</strong> {{ $project->projectManager?->name ?? '—' }}
            </td>
        </tr>
    </table>
    ...
    <table style="width: 100%; border: 1px solid #ddd; border-collapse: collapse; font-size: 8pt; margin-bottom: 12px;">
        <thead>
            <tr style="background-color: #f3f4f6;">
                ...
                <th style="border: 1px solid #ddd; padding: 5px 8px; text-align: left;">{{ __('Date / Scope') }}</th>
                <th style="border: 1px solid #ddd; padding: 5px 8px; text-align: right;">{{ __('Amount') }}</th>
            </tr>
        </thead>
        <tbody>
            <tr><td colspan="3" style="border: 1px solid #ddd; padding: 4px 8px; background-color: #fafafa; font-weight: bold; color: #555;">{{ __('Base Value') }}</td></tr>
            @foreach($revenueDetail['base_lines'] as $line)
                <tr>
                    <td style="border: 1px solid #ddd; padding: 4px 8px;">{{ $line['label'] }}</td>
                    <td style="border: 1px solid #ddd; padding: 4px 8px; color: #888;">—</td>
                    <td style="border: 1px solid #ddd; padding: 4px 8px; text-align: right;">${{ number_format($line['amount'], 2) }}</td>
                </tr>
            @endforeach
            <tr>
    ...
    <div style="font-size: 10pt; font-weight: bold; color: #3F5189; margin: 12px 0 4px 0;">{{ __('Per Job Site Breakdown') }}</div>
    <table style="width: 100%; border: 1px solid #ddd; border-collapse: collapse; font-size: 7.5pt; margin-bottom: 12px;">
        <thead>
            <tr style="background-color: #f3f4f6;">
                ...
                <th style="border: 1px solid #ddd; padding: 5px 6px; text-align: right;">{{ __('Contract Value') }}</th>
                <th style="border: 1px solid #ddd; padding: 5px 6px; text-align: right;">{{ __('Expenses') }}</th>
                <th style="border: 1px solid #ddd; padding: 5px 6px; text-align: right;">{{ __('Contracts') }}</th>
                <th style="border: 1px solid #ddd; padding: 5px 6px; text-align: right;">{{ __('Paid') }}</th>
                <th style="border: 1px solid #ddd; padding: 5px 6px; text-align: right;">{{ __('Unpaid') }}</th>
                <th style="border: 1px solid #ddd; padding: 5px 6px; text-align: right;">{{ __('Profit / Loss') }}</th>
            </tr>
        </thead>
        <tbody>
            @foreach($breakdown as $row)
                <tr>
                    <td style="border: 1px solid #ddd; padding: 4px 6px;">{{ $row['name'] }}</td>
                    <td style="border: 1px solid #ddd; padding: 4px 6px; text-align: right;">${{ number_format($row['contract_value'], 2) }}</td>
                    <td style="border: 1px solid #ddd; padding: 4px 6px; text-align: right;">${{ number_format($row['expenses'], 2) }}</td>
                    <td style="border: 1px solid #ddd; padding: 4px 6px; text-align: right;">${{ number_format($row['contracts_adjusted'], 2) }}</td>
                    <td style="border: 1px solid #ddd; padding: 4px 6px; text-align: right;">${{ number_format($row['contracts_paid'], 2) }}</td>
                    <td style="border: 1px solid #ddd; padding: 4px 6px; text-align: right;">${{ number_format($row['contracts_unpaid'], 2) }}</td>
                    <td style="border: 1px solid #ddd; padding: 4px 6px; text-align: right; font-weight: bold; color: {{ $row['profit'] < 0 ? '#e74c3c' : '#27ae60' }};">
                        {{ $row['profit'] < 0 ? '−' : '' }}${{ number_format(abs($row['profit']), 2) }}
                    </td>
                </tr>
            @endforeach
            <tr style="background-color: #f3f4f6; font-weight: bold;">
                <td style="border: 1px solid #ddd; padding: 5px 6px;">{{ __('Project Total') }}</td>
                <td style="border: 1px solid #ddd; padding: 5px 6px; text-align: right;">${{ number_format($financials['contract_value'], 2) }}</td>
                ...
                <td style="border: 1px solid #ddd; padding: 5px 6px; text-align: right; color: {{ $financials['profit'] < 0 ? '#e74c3c' : '#27ae60' }};">
                    {{ $financials['profit'] < 0 ? '−' : '' }}${{ number_format(abs($financials['profit']), 2) }}
                </td>
            </tr>
        </tbody>
    </table>
    ...
    {{-- Payment Schedule --}}
    @include('pdf.partials.cost-code-section', ['costCodes' => $costCodes, 'showLocation' => true])
    ...
    <table style="width: 100%; border: 1px solid #ddd; border-collapse: collapse; font-size: 7.5pt; margin-bottom: 12px;">
        <thead>
            <tr style="background-color: #f3f4f6;">
                ...
                <th style="border: 1px solid #ddd; padding: 5px 6px; text-align: left;">{{ __('Scope') }}</th>
        ...
        <tbody>
            @forelse($expenses as $expense)
                <tr>
                    ...
                    <td style="border: 1px solid #ddd; padding: 4px 6px;">{{ $expense->jobSite?->job_site_name ?? __('Project-level') }}</td>
            ...
            @empty
                <tr>
                    <td colspan="6" style="border: 1px solid #ddd; padding: 6px; text-align: center; color: #999; font-style: italic;">{{ __('No expenses recorded.') }}</td>
                </tr>
            @endforelse
            @if($expenses->count() > 0)
                <tr style="background-color: #f3f4f6; font-weight: bold;">
                    <td colspan="5" style="border: 1px solid #ddd; padding: 5px 6px; text-align: right;">{{ __('Total') }}</td>
                    <td style="border: 1px solid #ddd; padding: 5px 6px; text-align: right;">${{ number_format($financials['total_expenses'], 2) }}</td>
                </tr>
            @endif
        </tbody>
        ...
        <tbody>
            @forelse($contracts as $contract)
                ...
                <tr>
                    ...
                    <td style="border: 1px solid #ddd; padding: 4px 6px;">{{ $contract->jobSite?->job_site_name ?? __('Project-level') }}</td>
            ...
            @empty
                <tr>
                    <td colspan="7" style="border: 1px solid #ddd; padding: 6px; text-align: center; color: #999; font-style: italic;">{{ __('No contracts for this project.') }}</td>
                </tr>
            @endforelse
