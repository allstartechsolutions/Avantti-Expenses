        ...
        <div class="mb-6">
            ...
            <div class="mb-1">
                <button @click="toggleSubmenu('projects')"
                        class="flex items-center justify-between w-full px-3 py-2.5 text-sm font-medium {{ request()->routeIs('projects.*') || request()->routeIs('clients.*') ? 'text-[#3F5189] dark:text-[#4A5A96] bg-slate-100 dark:bg-slate-700' : 'text-slate-600 dark:text-slate-300' }} rounded-lg hover:bg-slate-100 dark:hover:bg-slate-700 group">
                    <div class="flex items-center">
                        <svg class="w-5 h-5 mr-3 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <a href="{{ route('suppliers.index') }}" class="flex items-center px-2.5 py-2.5 mb-1 text-sm font-medium {{ request()->routeIs('suppliers.*') ? 'text-white bg-gradient-to-r from-[#3F5189] to-[#4A5A96]' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700' }} rounded-lg group">
                ...
                <span x-show="!sidebarCollapsed || sidebarOpen" x-cloak>Suppliers</span>
            </a>
        </div>
    </nav>
