<div>
    ...
    <div class="mb-8">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-slate-900 dark:text-white">{{ __('Create Purchase Order') }}</h1>
            ...
            <div>
                <x-ui.button
                    variant="secondary"
                    href="{{ $jobSite ? route('jobsites.show', ['jobSite' => $jobSite->id, 'tab' => 'purchaseorders']) : route('projects.purchase-orders', $project->id) }}"
                    icon="arrow-left">
                    {{ __('Back') }}
                </x-ui.button>
            </div>
    ...
    <div class="space-y-8">
        ...
        <div class="bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700">
            <div class="px-6 py-4 border-b border-slate-200 dark:border-slate-700">
                ...
                <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">{{ __('Basic information about this purchase order') }}</p>
            </div>
            <div class="p-6 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                    ...
                    <div>
                        ...
                        @if($jobSite)
                            {{-- When coming from job site, show read-only field --}}
                            <div class="w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-md bg-slate-100 dark:bg-slate-600 text-slate-700 dark:text-slate-300">
                                {{ $jobSite->job_site_name }}
                            </div>
                        @else
                            {{-- When coming from project, allow location selection --}}
                            <select
                    ...
                    <!-- PO Number -->
                    <div>
                        <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">{{ __('PO Number') }}</label>
                        <input
                            type="text"
                ...
                <div>
                    ...
                    <textarea
                        ...
                        rows="2"
                        class="w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-[#3F5189] focus:border-[#3F5189] bg-white dark:bg-slate-700 text-slate-900 dark:text-white"
                        placeholder="{{ __('Optional notes about this purchase order...') }}"></textarea>
                </div>

                <!-- Quote/Document -->
                <div>
                    <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">{{ __('Quote/Document') }}</label>
                    <x-ui.file-drop
                        wire:model="po_receipt"
                        :multiple="false"
                        accept=".pdf,.jpg,.jpeg,.png"
                        :label="__('Drop the quote or document here, or')"
                        :hint="__('PDF, JPG or PNG, up to 10MB.')">

                        @error('po_receipt') <p class="text-sm text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
                        ...
                        @if($po_receipt)
                            <div class="flex items-center justify-between gap-3 px-3 py-2 text-sm border border-slate-200 dark:border-slate-700 rounded-lg">
                                <span class="min-w-0 flex-1 truncate text-slate-900 dark:text-white">
                                    {{ $po_receipt->getClientOriginalName() }}
                                </span>
                                <span class="shrink-0 text-xs text-slate-400 dark:text-slate-500">
                                    {{ \App\Services\DocumentSettings::formatBytes($po_receipt->getSize()) }}
                                </span>
                                <x-ui.icon-button
                                    variant="ghost"
                                    size="sm"
                                    icon="trash"
                                    type="button"
                                    wire:click="clearPoReceipt"
                                    title="{{ __('Remove :file', ['file' => $po_receipt->getClientOriginalName()]) }}"
                                    aria-label="{{ __('Remove :file', ['file' => $po_receipt->getClientOriginalName()]) }}"
            ...
            <div class="px-6 py-4 border-b border-slate-200 dark:border-slate-700">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">{{ __('Add the items for this purchase order') }}</p>
                    </div>
            ...
            <div class="p-6">
                ...
                @else
                    <div class="text-center py-12 bg-slate-50 dark:bg-slate-900/50 rounded-lg">
                        ...
                        <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">{{ __('Add at least one item to this purchase order.') }}</p>
                        <div class="mt-4">
                            <x-ui.button type="button" variant="primary" icon="plus" wire:click="openAddItemModal">
                                {{ __('Add Item') }}
                            </x-ui.button>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700">
            <div class="px-6 py-4 border-b border-slate-200 dark:border-slate-700">
                ...
                <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">{{ __('Payment settings (will be applied when PO is approved and expense is created)') }}</p>
            </div>
            <div class="p-6 space-y-6">
                ...
                <div class="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-900 rounded-lg">
                    <div>
                        ...
                        <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">{{ __('Enable to divide this into multiple payments when expense is created') }}</p>
                    </div>
                    <button
                        type="button"
        ...
        <div class="flex items-center justify-end space-x-4">
            ...
            <x-ui.button type="button" variant="outline" wire:click="saveAsDraft">
                {{ __('Save as Draft') }}
            </x-ui.button>
            <x-ui.button type="button" variant="primary" icon="paper-airplane" wire:click="saveAndSubmit">
