{{--
    Link an existing employee row to the same person's row at another company.
            ...
            <div class="sticky top-0 z-20 border-b border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800">
                <div class="mx-auto max-w-7xl px-6 py-4 flex items-center justify-between gap-4">
                    <div class="min-w-0">
                        ...
                        <p class="text-sm text-slate-500 dark:text-slate-400 truncate">{{ __('Say which record at another company is the same person. Both records stay as they are; they are counted as one person from now on.') }}</p>
                    </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div class="space-y-4">
                        ...
                        @if($linking->person)
                            <div class="{{ $card }} space-y-2">
                                ...
                                <p class="text-xs text-slate-500 dark:text-slate-400">{{ __('A new link joins this person; if the other record already belongs to someone else, the two people become one.') }}</p>
                            </div>
                        @endif
                    </div>
                    ...
                    <div class="lg:col-span-2 space-y-4">
                        @if($candidates['suggested']->isNotEmpty())
                            <div class="{{ $card }} space-y-3">
                                <h3 class="text-sm font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">{{ __('Looks like the same person') }}</h3>
                        ...
                        <div class="{{ $card }} space-y-3">
                            ...
                            @elseif($candidates['found']->isEmpty())
                                <p class="text-sm text-slate-500 dark:text-slate-400">{{ __('Nobody at another company matches that. The person may not have been added there yet — add them on that company\'s page and the link will be offered.') }}</p>
                            @else
                        ...
                        <div class="{{ $card }} space-y-2">
                            <label for="link_reason" class="{{ $label }}">{{ __('Why are these the same person?') }}</label>
                            <input type="text" id="link_reason" wire:model="link_reason" class="{{ $field }}" placeholder="{{ __('e.g. Same foreman, confirmed on site') }}">
                            <p class="text-xs text-slate-500 dark:text-slate-400">{{ __('Optional, but it is what the next person will read when they wonder who decided this.') }}</p>
            ...
            <div class="sticky bottom-0 z-20 border-t border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800">
                <div class="mx-auto max-w-7xl px-6 py-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <x-ui.button type="submit" variant="primary" icon="check" :disabled="! $link_target_id">{{ __('Link as the same person') }}</x-ui.button>
                    </div>
                </div>
            </div>
        </form>
