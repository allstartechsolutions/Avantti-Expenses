@php
    /*
     ...
     |--------------------------------------------------------------------------
     |
     | Drawn for whoever holds `dashboard.overview`. Every card and every panel
     | below is wrapped in the flag for the module it summarises, so this file
     | never has to think about roles: DashboardIndex::getBlocksProperty()
     | decided, and it also decided not to run the queries behind a block that
     | is off.
     |
     | Where two cards used to swap places depending on a module switch
     | (Receivables ↔ Over Budget, Open Estimates ↔ Open Purchase Orders) the
     | swap is kept exactly as it was, with the ability standing in for the
     | switch — so an admin's dashboard is unchanged by this pass.
     |
     | Every figure here is a roll-up over the whole company, which is what
     | `can_see_money` hides (M4), so each one goes through <x-ui.money rollup>.
     */
    ...
    // Only the chart needs this: <x-ui.money> formats its own.
    $currency = config('app.currency');

    $headerSubtitle = $blocks['invoices']
        ? __('Financial overview and pending actions')
        : __('Operations overview and pending actions');

    // The chart is nothing but money, so there is no version of it worth
    // drawing for somebody whose access hides totals.
    $showsChart = $blocks['money'] && ($blocks['expenses'] || $blocks['invoices']);
...
<div>
    ...
    <div class="mb-6 flex items-end justify-between gap-4 flex-wrap">
        ...
        @if ($blocks['expenses'] || $blocks['invoices'])
    ...
    @if ($showsAnyCard)
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                    ...
                    <div class="flex items-start justify-between">
                        <div>
                            ...
                            <p class="mt-2 text-2xl font-bold text-slate-900 dark:text-white">
                                <x-ui.money :amount="$kpis['cash_to_pay']" :visible="$blocks['money']" rollup />
                            </p>
                            <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">{{ __('Due this month') }}</p>
                        </div>
                        <div class="h-10 w-10 rounded-lg bg-red-100 dark:bg-red-900/30 flex items-center justify-center shrink-0">
                ...
                <div class="bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700 p-5">
                    <div class="flex items-start justify-between">
                        <div>
                            ...
                            <p class="mt-2 text-2xl font-bold text-slate-900 dark:text-white">
                                <x-ui.money :amount="$kpis['receivables']" :visible="$blocks['money']" rollup />
                            </p>
            ...
            @elseif ($blocks['over_budget'])
                {{-- Projects Over Budget (takes the Receivables slot when there are no invoices) --}}
                <div class="bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700 p-5">
                    <div class="flex items-start justify-between">
                        <div>
                        ...
                        <div class="h-10 w-10 rounded-lg bg-amber-100 dark:bg-amber-900/30 flex items-center justify-center shrink-0">
                ...
                <div class="bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700 p-5">
                    <div class="flex items-start justify-between">
                        <div>
                            ...
                            <p class="mt-2 text-2xl font-bold text-slate-900 dark:text-white">
                                <x-ui.money :amount="$kpis['open_estimates']" :visible="$blocks['money']" rollup />
                            </p>
            ...
            @elseif ($blocks['purchase_orders'])
                {{-- Open Purchase Orders (takes the Estimates slot when there are no estimates) --}}
                <div class="bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700 p-5">
                    <div class="flex items-start justify-between">
                        <div>
            ...
            @if ($blocks['projects'])
                ...
                <div class="bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700 p-5">
                    <div class="flex items-start justify-between">
                        ...
                        <div class="h-10 w-10 rounded-lg bg-indigo-100 dark:bg-indigo-900/30 flex items-center justify-center shrink-0">
    ...
    @if ($showsAnyPanel)
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
                            ...
                            <div class="px-5 py-3 flex items-center justify-between gap-3">
                                ...
                                <div class="text-right shrink-0">
                                    <p class="text-sm font-semibold text-slate-900 dark:text-white">
                                        <x-ui.money :amount="$payment->amount" :visible="$blocks['money']" rollup />
                                    </p>
                        ...
                        @forelse ($pastDueInvoices as $invoice)
                            ...
                            <a href="{{ route('invoices.show', $invoice->id) }}" class="block px-5 py-3 hover:bg-slate-50 dark:hover:bg-slate-700/40 transition">
                                <div class="flex items-center justify-between gap-3">
                                    ...
                                    <div class="text-right shrink-0">
                                        <p class="text-sm font-semibold text-slate-900 dark:text-white">
                                            <x-ui.money :amount="$invoice->getBalanceDue()" :visible="$blocks['money']" rollup />
                                        </p>
                                        <p class="text-xs text-red-600 dark:text-red-400">
            ...
            @elseif ($blocks['over_budget'])
                {{-- Over Budget Projects (takes the Past-Due Invoices slot when there are no invoices) --}}
                <div class="bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700 flex flex-col">
                            ...
                            <a href="{{ route('projects.overview', $project->id) }}" class="block px-5 py-3 hover:bg-slate-50 dark:hover:bg-slate-700/40 transition">
                                <div class="flex items-center justify-between gap-3">
                                    <div class="min-w-0">
                                        ...
                                        <p class="text-xs text-slate-500 dark:text-slate-400 truncate">
                                            {{ __('Budget') }}: <x-ui.money :amount="$contractValueDollars" :scope="$project" rollup />
                                        </p>
                                    </div>
                                    <div class="text-right shrink-0">
                                        <p class="text-sm font-semibold text-red-600 dark:text-red-400">
                                            +<x-ui.money :amount="$overageDollars" :scope="$project" rollup />
                                        </p>
            ...
            @if ($showsApprovals)
                ...
                <div class="bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700 flex flex-col">
                    ...
                    <div class="divide-y divide-slate-200 dark:divide-slate-700 flex-1">
                        ...
                        @foreach ($pendingApprovals['payment_batches'] as $batch)
        ...
        <div class="bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700 p-5"
            ...
            <div class="flex items-center justify-between mb-4">
                <h3 class="text-sm font-semibold text-slate-900 dark:text-white">
                    @if ($cashflow['show_inflow'] && $cashflow['show_outflow'])
                        {{ __('Cash Flow (Last 6 Months)') }}
                    @elseif ($cashflow['show_inflow'])
                        {{ __('Cash In (Last 6 Months)') }}
                    @else
                        {{ __('Cash Out (Last 6 Months)') }}
                    @endif
                </h3>
                <div class="flex items-center gap-4 text-xs">
                    @if ($cashflow['show_inflow'])
    ...
    @elseif (! $showsAnyCard && ! $showsAnyPanel)
        {{--
            The overview granted, but not one of the modules it summarises.
            Say so rather than leaving the page blank — this is somebody whose
            access needs adjusting, and the message is what tells their
            administrator that.
        --}}
        <div class="bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700 p-10 text-center">
            <svg class="mx-auto h-10 w-10 text-slate-300 dark:text-slate-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 17V7m4 10V11m4 6V9M5 21h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v14a2 2 0 002 2z" />
            </svg>
            <h2 class="mt-3 text-base font-semibold text-slate-900 dark:text-white">{{ __('Nothing to summarise yet') }}</h2>
            <p class="mt-1 text-sm text-slate-500 dark:text-slate-400 max-w-md mx-auto">
                {{ __('You can see the overview, but none of the areas it reports on — expenses, invoices, estimates, projects or purchase orders — are open to you. Ask an administrator to add one.') }}
            </p>
        </div>
    @endif
