    ...
    <div class="mb-8">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">{{ __('Manage your invoices and payments') }}</p>
            </div>
            <div>
                <x-ui.button
                    variant="primary"
        ...
        <div class="border-b border-slate-200 dark:border-slate-700">
            <nav class="-mb-px flex space-x-6" aria-label="{{ __('Tabs') }}">
                ...
                <button wire:click="setStatusFilter('pending')"
                    ...
                    {{ __('Pending') }} <span class="ml-1 text-xs bg-yellow-100 dark:bg-yellow-700 text-yellow-600 dark:text-yellow-300 px-2 py-0.5 rounded-full">{{ $statusCounts['pending'] }}</span>
                </button>
                <button wire:click="setStatusFilter('partial')"
                    ...
                    {{ __('Partial') }} <span class="ml-1 text-xs bg-orange-100 dark:bg-orange-700 text-orange-600 dark:text-orange-300 px-2 py-0.5 rounded-full">{{ $statusCounts['partial'] }}</span>
                </button>
                <button wire:click="setStatusFilter('paid')"
                    ...
                    {{ __('Paid') }} <span class="ml-1 text-xs bg-green-100 dark:bg-green-700 text-green-600 dark:text-green-300 px-2 py-0.5 rounded-full">{{ $statusCounts['paid'] }}</span>
                </button>
                <button wire:click="setStatusFilter('past_due')"
                    ...
                    {{ __('Past Due') }} <span class="ml-1 text-xs bg-red-100 dark:bg-red-700 text-red-600 dark:text-red-300 px-2 py-0.5 rounded-full">{{ $statusCounts['past_due'] }}</span>
                </button>
            </nav>
        </div>
    </div>
        ...
        <div class="p-6">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div class="flex-1 max-w-md">
                    <label for="search" class="sr-only">{{ __('Search invoices') }}</label>
                    <div class="relative">
        ...
        @else
            ...
            <div class="text-center py-12">
                ...
                <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">
                    ...
                    @else
                        {{ __('Get started by creating a new invoice.') }}
                    @endif
                </p>
                @if(!$search && !$statusFilter)
                    <div class="mt-6">
