            ...
            <div class="bg-gradient-to-r from-[#3F5189] to-[#4A5A96] rounded-lg shadow-sm p-6 text-white">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <x-ui.money class="block text-2xl font-bold mt-1" :amount="$totalIncomeAmount" :scope="$jobSite" rollup />
                    </div>
            ...
            <div class="bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700 p-6">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <x-ui.money class="block text-2xl font-bold mt-1 text-green-600 dark:text-green-400" :amount="$thisMonthAmount" :scope="$jobSite" rollup />
                    </div>
            ...
            <div class="bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700 p-6">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <x-ui.money class="block text-2xl font-bold mt-1 text-amber-600 dark:text-amber-400" :amount="$expectedAmount" :scope="$jobSite" rollup />
                        @if($overdueAmount > 0)
                            <p class="text-xs text-red-600 dark:text-red-400 mt-1">
                                {{ __('Overdue') }}: <x-ui.money :amount="$overdueAmount" :scope="$jobSite" rollup />
                            </p>
                        @endif
                    </div>
        ...
        @else
            <div class="bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700">
                <div class="p-12 text-center">
                    ...
                    @else
                        <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">{{ __('Nothing has been recorded here yet. You can see income on this job site but not add it — ask an administrator if that is wrong.') }}</p>
                    @endcan
                </div>
            </div>
        @endif
