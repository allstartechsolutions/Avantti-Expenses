<x-jobsite-layout :jobSite="$jobSite" active="report" :title="__('Financial Report')">
    <x-slot:actions>
        <x-ui.button
            variant="secondary"
            href="{{ route('jobsites.report.pdf.view', $jobSite) }}"
            icon="eye"
            target="_blank">
            {{ __('View PDF') }}
        </x-ui.button>
        <x-ui.button
            variant="primary"
            href="{{ route('jobsites.report.pdf.download', $jobSite) }}"
    ...
    <div class="bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700 mb-6">
        ...
        <div class="px-6 py-4">
            <div class="flex justify-between py-2 text-sm">
                <span class="text-slate-700 dark:text-slate-300">{{ __('Job Amount') }}</span>
    ...
    <!-- Payment Schedule -->
    @include('livewire.shared.cost-code-section', ['costCodes' => $costCodes, 'showLocation' => false])
    ...
    <div class="bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700 mb-6">
        ...
        @if ($expensesDetail->isEmpty())
            <div class="px-6 py-8 text-center text-sm text-slate-500 dark:text-slate-400">
                {{ __('No expenses recorded for this job site.') }}
            </div>
        @else
    ...
    <div class="bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700 mb-6">
        ...
        @if ($contractsDetail->isEmpty())
            <div class="px-6 py-8 text-center text-sm text-slate-500 dark:text-slate-400">
                {{ __('No contracts for this job site.') }}
            </div>
        @else
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-slate-200 dark:divide-slate-700">
                    ...
                    <tfoot class="bg-slate-100 dark:bg-slate-700/60">
                        <tr class="font-semibold">
                            <td colspan="3" class="px-6 py-3 text-sm text-slate-900 dark:text-white">{{ __('Total') }}</td>
                            <td class="px-6 py-3 text-sm text-right text-slate-900 dark:text-white">
