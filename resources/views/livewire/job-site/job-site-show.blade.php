<x-jobsite-layout :jobSite="$jobSite" :active="$activeNavTab" title="{{ __('Job Site Details') }}">
    <x-slot:actions>
    ...
    <div>
                    ...
                    <div class="bg-gradient-to-r from-[#3F5189] to-[#4A5A96] rounded-lg shadow-sm p-6 text-white">
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <x-ui.money class="block text-2xl font-bold mt-1" :amount="$totalExpensesAmount" :scope="$jobSite" rollup />
                            </div>
                    ...
                    <div class="bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700 p-6">
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <x-ui.money class="block text-2xl font-bold mt-1 text-green-600 dark:text-green-400" :amount="$totalPaidAmount" :scope="$jobSite" rollup />
                            </div>
                    ...
                    <div class="bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700 p-6">
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <x-ui.money class="block text-2xl font-bold mt-1 text-amber-600 dark:text-amber-400" :amount="$totalPendingAmount" :scope="$jobSite" rollup />
                            </div>
                ...
                @else
                    <div class="bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700">
                        <div class="p-12 text-center">
                            ...
                            @else
                                <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">{{ __('Nothing has been recorded here yet. You can see expenses on this job site but not add them — ask an administrator if that is wrong.') }}</p>
                            @endcan
                        </div>
                    </div>
                @endif
        ...
        @if($activeTab === 'changeorders')
            <div class="space-y-6">
                ...
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    ...
                    @can('change-orders.create', $jobSite)
        ...
        @if($activeTab === 'dailyreports')
            <div>
                <div class="flex items-center justify-between mb-6">
                    <h2 class="text-lg font-semibold text-slate-900 dark:text-white">{{ __('Daily Reports') }}</h2>
                    <x-ui.button
                        variant="primary"
                        icon="plus"
                                ...
                                <div class="p-6">
                                        ...
                                        <div class="flex-1">
                                            <div class="flex items-center space-x-3 mb-2">
                                                <h3 class="text-lg font-semibold text-slate-900 dark:text-white">
                                                    Daily Report - {{ $report->report_date->appDate() }}
                                                </h3>
                                    ...
                                    @if($report->tasks->count() > 0)
                                        <div class="mt-4 pt-4 border-t border-slate-200 dark:border-slate-700">
                                            <h4 class="text-sm font-medium text-slate-700 dark:text-slate-300 mb-3">{{ __('Tasks:') }}</h4>
                                            <div class="space-y-2">
                ...
                @else
                    <div class="bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700 p-12 text-center">
                        ...
                        <h3 class="mt-2 text-sm font-medium text-slate-900 dark:text-white">{{ __('No daily reports yet') }}</h3>
                        <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">{{ __('Get started by creating your first daily report.') }}</p>
                        <div class="mt-6">
                            <x-ui.button
                                variant="primary"
                                href="{{ route('dailyreports.create', $jobSite) }}"
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700">
                    <div class="px-6 py-4 border-b border-slate-200 dark:border-slate-700 flex items-center justify-between">
                        ...
                        @can('purchase-orders.create', $jobSite)
                            <x-ui.button
                                variant="primary"
                                size="sm"
                    ...
                    @else
                        <div class="text-center py-12">
                            ...
                            @else
                                <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">{{ __('Nothing has been ordered here yet. You can see purchase orders on this job site but not raise them — ask an administrator if that is wrong.') }}</p>
                            @endcan
                        </div>
                    @endif
                </div>
        ...
        @if($activeTab === 'budget')
            <div class="space-y-6">
                <div class="bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700">
                    <div class="px-6 py-4 border-b border-slate-200 dark:border-slate-700 flex items-center justify-between">
                        <h3 class="text-lg font-semibold text-slate-900 dark:text-white">{{ __('Job Site Budget') }}</h3>
                        @if(!$budget)
                            <x-ui.button
                                variant="primary"
                                size="sm"
                    ...
                    <div class="p-6">
                        @if($budget)
                            ...
                            @if($budget->parentItems->count() > 0)
                                <div class="mt-6">
                                    <h5 class="text-sm font-medium text-slate-700 dark:text-slate-300 mb-3">{{ __('Cost Code Summary') }}</h5>
                                    <div class="space-y-2">
                        ...
                        @else
                            <div class="text-center py-8">
                                ...
                                <h4 class="mt-2 text-sm font-medium text-slate-900 dark:text-white">{{ __('No budget yet') }}</h4>
                                <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">{{ __('Create a budget to track cost allocation for this job site.') }}</p>
                            </div>
                        @endif
                    </div>
                </div>
                ...
                <div class="space-y-4">
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <p class="text-slate-900 dark:text-white">{{ \Carbon\Carbon::parse($expense_date)->appDate() }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">{{ __('Type') }}</label>
                            <p class="text-slate-900 dark:text-white">
                                @if($isCustomItem)
                                    <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-amber-100 text-amber-800 dark:bg-amber-900/20 dark:text-amber-300">{{ __('Custom Item') }}</span>
                                @else
                                    <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-blue-100 text-blue-800 dark:bg-blue-900/20 dark:text-blue-300">{{ __('From Catalog') }}</span>
                                @endif
                            </p>
                        </div>
                    </div>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">{{ __('Item Name') }}</label>
                        <p class="text-slate-900 dark:text-white">{{ $expense_item_name }}</p>
                    </div>

                    <div class="grid grid-cols-3 gap-4">
                        <div>
                            <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">{{ __('Quantity') }}</label>
                            <p class="text-slate-900 dark:text-white">{{ $expense_quantity }}{{ $expense_usage_unit ? ' ' . $expense_usage_unit : '' }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">{{ __('Unit Price') }}</label>
                                                    ...
                                                    <td class="px-4 py-2 text-right">
                                                        ...
                                                        @elseif($payment->status === 'pending')
                                                            @can('expenses.pay', $viewingExpense)
                                                                <button
                                                                    wire:click="startMarkPaid('payment', {{ $payment->id }})"
            ...
            @else
                ...
                <form wire:submit.prevent="saveExpense" class="space-y-4">
                    ...
                    @if(!$isCustomItem)
                        <!-- Catalog Item Search -->
                        <div class="relative">
                            <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">{{ __('Search Catalog Item') }}</label>
                            <input
                                type="text"
                    ...
                    @if(!$isCustomItem && $selectedCatalogItem && $expense_purchase_unit && $expense_usage_unit)
                        ...
                        <div class="bg-blue-50 dark:bg-blue-900/10 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
                            <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-3">
                                {{ __('Select Unit Type') }} <span class="text-red-500">*</span>
                            </label>
                            <div class="grid grid-cols-2 gap-3">
                                <label class="relative flex items-center p-3 border-2 rounded-lg cursor-pointer transition-all {{ $expense_unit_type_used === 'usage' ? 'border-[#3F5189] bg-[#3F5189]/5' : 'border-slate-300 dark:border-slate-600 hover:border-slate-400' }}">
                                    ...
                                    <div class="flex-1">
                                        ...
                                        <div class="text-xs text-slate-500 dark:text-slate-400 mt-1">{{ __('Usage unit') }}</div>
                                    </div>
                                </label>
                                <label class="relative flex items-center p-3 border-2 rounded-lg cursor-pointer transition-all {{ $expense_unit_type_used === 'purchase' ? 'border-[#3F5189] bg-[#3F5189]/5' : 'border-slate-300 dark:border-slate-600 hover:border-slate-400' }}">
                                    ...
                                    <div class="flex-1">
                                        ...
                                        <div class="text-xs text-slate-500 dark:text-slate-400 mt-1">{{ __('Purchase unit') }}</div>
                                    </div>
                                </label>
                            </div>
                    ...
                    @if($isCustomItem)
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">{{ __('Type') }}</label>
                        ...
                        @if($existingReceiptPath && !$expense_receipt)
                            <div class="mb-2 p-2 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
                                <div class="flex items-center justify-between text-sm">
                                    <span class="text-slate-600 dark:text-slate-400">{{ __('Current receipt:') }}</span>
                        ...
                        @if(!$expense_has_installments)
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                ...
                                @else
                                    <div>
                                        <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">
                                            {{ __('Payment Due Date') }} <span class="text-red-500">*</span>
                                        </label>
                        ...
                        @else
                            ...
                            <div class="space-y-4">
                                <div class="grid grid-cols-3 gap-4">
                                    <div>
                                        <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">
                                            {{ __('Number of Installments') }} <span class="text-red-500">*</span>
                                        </label>
                                        <input
                                            type="number"
                                            min="2"
                                    ...
                                    <div>
                                        <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">
                                            {{ __('Frequency') }} <span class="text-red-500">*</span>
                                        </label>
                                        <select
                                    ...
                                    <div>
                                        <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">
                                            {{ __('First Payment Date') }} <span class="text-red-500">*</span>
                                        </label>
                                ...
                                <div class="flex items-center space-x-4">
                                    ...
                                    <label class="flex items-center cursor-pointer">
                                        <input
                                            ...
                                            value="1"
                                            class="border-slate-300 dark:border-slate-600 text-[#3F5189] focus:ring-[#3F5189]">
                                        <span class="ml-2 text-sm text-slate-700 dark:text-slate-300">{{ __('Custom amounts') }}</span>
                                    </label>
                                </div>
                                ...
                                @if(count($expense_payment_schedule_preview) > 0)
                                    <div class="bg-slate-50 dark:bg-slate-900 rounded-lg p-4">
                                        <h4 class="text-sm font-medium text-slate-700 dark:text-slate-300 mb-3">{{ __('Payment Schedule Preview') }}</h4>
                                        <div class="max-h-48 overflow-y-auto">
                                            <table class="min-w-full divide-y divide-slate-200 dark:divide-slate-700">
                                                <thead>
                                                    <tr>
                                                        <th class="px-3 py-2 text-left text-xs font-medium text-slate-500 dark:text-slate-400 uppercase">#</th>
                                                        <th class="px-3 py-2 text-left text-xs font-medium text-slate-500 dark:text-slate-400 uppercase">{{ __('Due Date') }}</th>
                                                        <th class="px-3 py-2 text-left text-xs font-medium text-slate-500 dark:text-slate-400 uppercase">{{ __('Amount') }}</th>
                                                    </tr>
                                                </thead>
                                                <tbody class="divide-y divide-slate-200 dark:divide-slate-700">
