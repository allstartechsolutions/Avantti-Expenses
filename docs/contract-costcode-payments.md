...
5. **Project/lot cost-code grid** — ✅ done 2026-08-10. `Budget::costCodeGrid()` + full-page `BudgetCostGrid` at `budgets/{budget}/cost-grid` ("Cost Grid" button on the budget page). Excel-like: sections → lines, section Sub-Totals with "% of budget", Unassigned row only when no default is set, summary cards + grand totals. Columns Budgeted | Contracted | Paid | % Complete | Balance (= contracted − paid). % Complete is weighted by scheduled value across that code's contracts (contracts with no recorded % are excluded from the weighting). Aggregates contracts of the budget's exact location (project + lot pair, cancelled excluded) through each contract's `costCodeSchedule()`, so the default-code fallback applies everywhere.
